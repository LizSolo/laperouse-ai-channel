...
# Витрина режима отбора: кандидаты для Лизы. День, когда пришла она, а постов нет,
# добавлен 2026-08-13 пятым статусом. Без него такой день неотличим от сбоя: три
# файла из WATCHED за сегодня не менялись, и сторож поднимал бы тревогу на
# штатном исходе — так же, как поднимал её на пустом дне до 06.08.
...
def main() -> int:
    ...
    # Витрина без постов: тоже законный исход, но молчать о нём нельзя — Лиза
    # ждёт от прогона либо посты, либо объяснение. Проверяется после пустого дня
    # и до остальных: в такой день selfcheck и разбор не пишутся.
    if last_commit_date(SHORTLIST_FILE) == today and last_commit_date("DRAFTS.md") != today:
        send(
            token,
            chat_id,
            "ℹ️ Прогон состоялся: витрина кандидатов за сегодня пришла, "
            "готовых постов в ней нет.\n\n"
            "Назови номера тем, которые развернуть в посты, — это не сбой.",
        )
        print(f"Витрина без постов: {SHORTLIST_FILE} обновлён сегодня ({today})")
        return 0
